...
class FirestoreRepository {
        ...
        return try {
            ...
            if (document.exists()) {
                ...
                UserRecord(
                    ...
                    lastRingClosedDate = document.getTimestamp("lastRingClosedDate"),
                    ...
                    accountCreated = document.getTimestamp("accountCreated")
                )
            } else {
                null
            }
    ...
    suspend fun saveFluidLog(userId: String, log: FluidLog): Result<Unit> {
        return try {
            // Update totalFluidDrankAllTime when saving a log
    ...
    suspend fun incrementTotalRingsClosed(userId: String): Result<Unit> {
        return try {
            db.runTransaction { transaction ->
                val userRef = usersCollection.document(userId)
                val userDoc = transaction.get(userRef)
                ...
                transaction.update(userRef, "lastRingClosedDate", Timestamp.now())
            }.await()
            Result.success(Unit)
        } catch (e: Exception) {
            Result.failure(e)
    ...
    suspend fun softDeleteUser(userId: String): Result<Unit> {
        return try {
            usersCollection.document(userId).update("isDeleted", true).await()
            Result.success(Unit)
        } catch (e: Exception) {
            Result.failure(e)
        }
