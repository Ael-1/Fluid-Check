...
class SmartReminderWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        val fluidLogRepository = FluidLogRepository(applicationContext)
        val auth = com.google.firebase.auth.FirebaseAuth.getInstance()
        val userId = inputData.getString("user_id") ?: auth.currentUser?.uid ?: "GUEST"
        
        val notifRes = prefsRepository.getNotificationsEnabled(userId).first()
        val notificationsEnabled = (notifRes as? com.example.fluidcheck.util.DataResult.Success)?.data
        if (notificationsEnabled == false) return Result.success()

        val userRecordRes = userRepository.getUserRecord(userId)
        val userRecord = (userRecordRes as? com.example.fluidcheck.util.DataResult.Success)?.data ?: return Result.success()
        val dailyGoal = userRecord.dailyGoal ?: 3000
        
        val todayLogsResult = fluidLogRepository.getTodayFluidLogs(userId)
        val todayLogs = if (todayLogsResult is com.example.fluidcheck.util.DataResult.Success) todayLogsResult.data else emptyList()
